...
```{r getinput, echo=FALSE, cache=FALSE, opts.label='shiny'}
     library(shiny); library(shinyapps); library(UsingR)
     data(mtcars)
     app <- shinyApp(
          ui = bootstrapPage(
               numericInput( "cyl2", label = "No of cylinders",
                             value = mean(mtcars$cyl)),
               numericInput("wt2", label = "Weight", value = mean(mtcars$wt)),
               textOutput("p1")
          ),
          server = function(input, output) {
               output$p1 <- renderText (
                   paste ("your mpg is",
                           predict(predModel, 
                                   data.frame(cbind(cyl=input$cyl2,
                                                    wt=input$wt2)
                                              ),
